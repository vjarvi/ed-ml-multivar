...
\documentclass[english, ieee]{tauthesis}
...
\usepackage{tabularx}
\usepackage[final]{microtype} % better line breaking and protrusion
\emergencystretch=1em % modest stretch to avoid tiny overfull hboxes
...
\title{Foundational time series models for emergency department occupancy forecasting}{}
% \subtitle{Emergency department occupancy forecasting}{}
...
% separate line.
\examiner[Examiner]{M.Sc. Eetu Pulkkinen}
...
\thesistype{Bachelor's Thesis}{Kandidaatin tutkielma}

% The faculty and degree programme names in
% the primary and the secondary languages of
% the thesis.
\facultyname{Faculty of Information Technology and Communication Sciences}{Informaatioteknologian ja viestinnän tiedekunta}
\programmename{Bachelor’s Programme in Computing and Electrical Engineering}{Tieto- ja sähkötekniikan kandidaattiohjelma}

% The keywords to the thesis in the primary and
% the secondary languages of the thesis
\keywords%
    {time series forecasting, emergency department occupancy forecasting, time series foundational models, foundation models}
    {aikasarjaennustaminen, Päivystyspoliklinikoiden kuormituksen ennustus, esikoulutetut aikasarjamallit, perustamallit}
...
% AI usage disclosure
\clearpage
\chapter*{USE OF AI IN THESIS}
\input{tex/useofai.tex} 
\clearpage
...
\chapter{Computational costs for multivariate forecasts}
\label{ch:latency_by_subset}
\input{../output/tables/latency_by_subset.tex}

\label{ch:ram_by_sybset}
